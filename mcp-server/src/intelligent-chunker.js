...
class IntelligentChunker {
     ...
     */
    chunkBySections(content, metadata) {
    ...
    createMetadataHeader(metadata) {
        ...
        let header = '';
        if (metadata.socketType)
            header += `Socket Type: ${metadata.socketType}\n`;
        if (metadata.application)
            header += `Application: ${metadata.application}\n`;
        if (metadata.marketSegment)
            header += `Market Segment: ${metadata.marketSegment}\n`;
        if (header.length > 0)
            header += '\n';
        return header;
    }
     ...
     */
    createChunk(content, sectionTitle, index, metadata, metadataHeader) {
        return {
     ...
     */
    chunkDocument(content, filename, metadata) {
        const fullMetadata = {
            filename,
            ...metadata
        };
